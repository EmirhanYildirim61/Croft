...
export default function SubscriptionsScreen({ accounts }: Props) {
  ...
  const handleSkip = async (id: number, label: string) => {
    try {
      ...
      showToast(`Skipped "${label}".`, 'success');
      await load();
    } catch (e) {
      showToast(String(e), 'error');
    }
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          <p className="text-xs text-slate-400 mt-1">{subscriptions.length} active subscription{subscriptions.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <p className="text-sm text-slate-500 mb-1">Monthly Average</p>
          <p className="text-3xl font-bold tabular-nums text-red-600">{formatCents(totalMonthly)}</p>
          <p className="text-xs text-slate-400 mt-1">estimated from recurring items</p>
        </div>
      </div>
      ...
      <section className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100">
          <h2 className="text-base font-semibold text-slate-700">All Subscriptions</h2>
        </div>
        {subscriptions.length === 0 ? (
          <p className="text-slate-400 text-sm p-6">
            No expense recurring items yet. Add them in Settings → Recurring Items.
          </p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Frequency</th>
                <th className="text-right px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Amount</th>
                <th className="text-right px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Annual Cost</th>
                  ...
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      {isDueSoon(item.next_due_date) && (
                        <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium">Soon</span>
                      )}
                    </div>
                    <p className="text-xs text-slate-400">{accName(item.account_id)}</p>
                  </td>
                  <td className="px-6 py-3 text-slate-600">{FREQ_LABEL[item.frequency] ?? item.frequency}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-slate-200 bg-slate-50">
                ...
                <td className="px-6 py-3 text-right font-bold tabular-nums text-red-600">
                  {formatCents(totalMonthly)}<span className="text-xs font-normal text-slate-400">/mo</span>
                </td>
                <td className="px-6 py-3 text-right font-bold tabular-nums text-red-600">
                  {formatCents(totalAnnual)}<span className="text-xs font-normal text-slate-400">/yr</span>
                </td>
                <td colSpan={2} />
              </tr>
            </tfoot>
